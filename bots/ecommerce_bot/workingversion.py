...
from bots.ecommerce_bot.metrics import RAGMetrics
...
Response:"""
    ...
    async def chat(self, message: str, user_id: Optional[str] = None, conversation_history: Optional[List[Dict]] = None) -> Dict[str, Any]:
        ...
        self.logger.info(f"Response generated successfully ({len(response)} chars) in {response_time_ms:.0f}ms")
        self.logger.info(f"📊 Metrics - Success Rate: {self.metrics.get_summary()['success_rate']}%, Avg Response: {self.metrics.get_summary()['avg_response_time_ms']}ms")
